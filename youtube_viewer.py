...
referers = ['https://search.yahoo.com/', 'https://duckduckgo.com/', 'https://www.google.com/',
            'https://www.bing.com/', 'https://t.co/', 'https://slivator.com/d/5-Создам-форум-или-сайт-под-ключ']
...
def clean_exe_temp(folder):
    ...
    if hasattr(sys, '_MEIPASS'):
        ...
        if sys.version_info.minor < 7 or sys.version_info.minor > 9:
            print(
                f'Ваша текущая версия python несовместима : {sys.version}')
            print(f'Установите версию Python от 3.7.x до 3.9.x, чтобы запустить этот скрипт')
            input("")
            sys.exit()
...
def update_chrome_version():
    link = 'https://slivator.com/versions.txt'
...
def check_update():
    ...
    if RELEASE_VERSION > SCRIPT_VERSION:
        ...
        print(bcolors.OKCYAN + 'Доступно обновление!!! ' +
              f'Версия для просмотра на YouTube {SCRIPT_VERSION} необходимо обновить до {RELEASE_VERSION} версии.' + bcolors.ENDC)
...
def direct_or_search(position):
    ...
    if position % 2:
        try:
            ...
            raise Exception("Заполните файл urls.txt")
        ...
        try:
            ...
            try:
                ...
                raise Exception("Заполните файл search.txt")
...
def update_view_count(position):
    ...
    print(timestamp() + bcolors.OKCYAN +
          f'Рабочий {position} | добавленный вид : {view_count}' + bcolors.ENDC)

    create_html({"#29b2d3": f'Рабочий {position} | добавленный вид : {view_count}'})
...
def youtube_normal(method, keyword, video_title, driver, output):
    ...
    try:
        ...
        raise Exception(
            "Низкая скорость интернета или застревание в reCAPTCHA! Не удается загрузить YouTube...")
...
def youtube_music(driver):
    if 'coming-soon' in driver.title or 'not available' in driver.title:
        raise Exception(
            "Музыка с YouTube недоступна в вашем регионе!")
    try:
        WebDriverWait(driver, 10).until(EC.visibility_of_element_located(
...
def control_player(driver, output, position, proxy, youtube, collect_id=True):
    ...
    if video_len == 0:
        raise Exception('Видеоплеер не загружается...')
    ...
    for _ in range(loop):
        ...
        if error == 10:
            error_msg = f'Воспроизведение видео занимает слишком много времени | Причина : буферизация'
            if current_state == -1:
                error_msg = f"Не удалось воспроизвести видео | Возможная причина : {proxy.split('@')[-1]} больше не работает"
...
def main_viewer(proxy_type, proxy, position):
    ...
    try:
        ...
        try:
            print(timestamp() + bcolors.OKBLUE + f"Worker {position} | " + bcolors.OKGREEN +
                  f"{proxy} | {proxy_type.upper()} | Хороший прокси | Открывания нового видео..." + bcolors.ENDC)
...
def stop_server(immediate=False):
    if not immediate:
        print('На завершение всех запущенных драйверов отводится максимум 15 минут...')
        for _ in range(180):
            sleep(5)
    ...
    if api:
        for _ in range(10):
            ...
            if response.status_code == 200:
                ...
                print(f'Ошибка завершения работы сервера : {response.status_code}')
                sleep(3)


def clean_exit():
    print(timestamp() + bcolors.WARNING +
          'Процессы очистки...' + bcolors.ENDC)
    create_html({"#f3f342": "Процессы очистки..."})
...
def main():
    ...
    if category != 'r':
        print(bcolors.OKCYAN +
              f'Всего прокси : {len(proxy_list)}' + bcolors.ENDC)
        ...
        try:
            while not_done:
                ...
                if len(view) >= views:
                    ...
                    hash_config = get_hash(config_path)
                    print(timestamp() + bcolors.WARNING +
                          'Измененный файл config.json скоро вступит в силу...' + bcolors.ENDC)
                    create_html(
                        {"#f3f342": 'Измененный файл config.json скоро вступит в силу...'})
                    ...
                    if (time() - start_time) > refresh*60:
                        ...
                        if sorted(proxy_list_new) != sorted(proxy_list_old):
                            print(timestamp() + bcolors.WARNING +
                                  f'Refresh {refresh} сработала минута. Прокси-серверы будут перезагружены в ближайшее время...' + bcolors.ENDC)
                            create_html(
            ...
            print(timestamp() + bcolors.WARNING +
                  'Держись!!! Позвольте мне на минутку закрыть все запущенные драйверы.' + bcolors.ENDC)
            create_html(
                {"#f3f342": 'Держись!!! Позвольте мне на минутку закрыть все запущенные драйверы.'})
...
if __name__ == '__main__':
    ...
    if os.path.isfile(config_path):
        ...
        if len(config) == 11:
            ...
            print(bcolors.OKCYAN + 'Конфигурационный файл существует! Программа запустится автоматически через 20 секунд...' + bcolors.ENDC)
            print(bcolors.FAIL + 'Если вы хотите создать новый конфигурационный файл, нажмите CTRL + C в течение 20 секунд!' + bcolors.ENDC)
            start = time()
            try:
                i = 0
                while i < 96:
                    print(bcolors.OKBLUE + f"{time() - start:.0f} оставшиеся секунды " +
            ...
            print(bcolors.FAIL + 'Предыдущий конфигурационный файл несовместим с последним скриптом! Создайте новый...' + bcolors.ENDC)
            create_config(config_path=config_path)
    else:
        create_config(config_path=config_path)
    ...
    while len(view) < views:
        try:
            ...
            if auth_required and background:
                print(bcolors.FAIL +
                      "Премиум прокси нуждается в расширении для работы. Chrome не поддерживает расширение в безголовом режиме." + bcolors.ENDC)
                input(bcolors.WARNING +
                      f"Либо используйте прокси-сервер без имени пользователя и пароля, либо отключите безголовый режим" + bcolors.ENDC)
                sys.exit()

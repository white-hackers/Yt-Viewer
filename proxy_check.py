...
def load_proxy():
    ...
    filename = input(bcolors.OKBLUE +
                     'Введите имя вашего файл с proxy: ' + bcolors.ENDC)
...
def main_checker(proxy_type, proxy, position):
    ...
    try:
        ...
        print(bcolors.OKBLUE + f"Worker {position+1} | " + bcolors.OKGREEN +
              f'{proxy} | Хороший | Тип : {proxy_type} | Ответ : {status}' + bcolors.ENDC)
        ...
        print(bcolors.OKBLUE + f"Worker {position+1} | " + bcolors.FAIL +
              f'{proxy} | {proxy_type} | Плохой | {e}' + bcolors.ENDC)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        try:
            ...
            print(bcolors.WARNING +
                  'Держись!!! Позвольте мне на минутку закончить запущенные потоки' + bcolors.ENDC)
            cancel_all = True
...
if __name__ == '__main__':
    ...
    try:
        threads = int(
            input(bcolors.OKBLUE+'Потоки (рекомендуем = 100): ' + bcolors.ENDC))
    except Exception:
        threads = 100
    ...
    print(bcolors.OKCYAN +
          f'Всего уникальных прокси : {total_proxies}' + bcolors.ENDC)
